...
#Rack2
resource "nsxt_policy_vlan_segment" "fd2_bgp_segments" {
  for_each = var.edge_node_bgp_segments["rack1"]

  display_name        = each.value[0]
  description         = "Rack 2 ${each.value[0]}"
  vlan_ids            = [each.value[1]]
  transport_zone_path = data.nsxt_policy_transport_zone.uplink_tz.path
  advanced_config {
    uplink_teaming_policy = each.value[2]
...
data "nsxt_policy_edge_node" "edge_nodes_fd2" {
  for_each          = var.edge_vms["rack2"]
  edge_cluster_path = data.nsxt_policy_edge_cluster.edge_cluster.path
  display_name      = each.key
}
...
#Rack2
resource "nsxt_policy_tier0_gateway_interface" "fd2_edge_vm_uplink_if1" {
  for_each       = var.edge_vms["rack2"]
  display_name   = "${each.value[0]}_uplink_tor1"
  description    = "${each.value[0]} uplink to TOR1"
  type           = "EXTERNAL"
  edge_node_path = data.nsxt_policy_edge_node.edge_nodes_fd2[each.key].path
  gateway_path   = nsxt_policy_tier0_gateway.infra_t0.path
  segment_path   = nsxt_policy_vlan_segment.fd2_bgp_segments["bgp_tor_a"].path
  subnets        = [each.value[1]]
  mtu            = var.edge_node_uplink_mtu
}

resource "nsxt_policy_tier0_gateway_interface" "fd2_edge_vm_uplink_if2" {
  for_each       = var.edge_vms["rack2"]
  display_name   = "${each.value[0]}_uplink_tor2"
  description    = "${each.value[0]} uplink to TOR2"
  type           = "EXTERNAL"
  edge_node_path = data.nsxt_policy_edge_node.edge_nodes_fd2[each.key].path
  gateway_path   = nsxt_policy_tier0_gateway.infra_t0.path
  segment_path   = nsxt_policy_vlan_segment.fd2_bgp_segments["bgp_tor_b"].path
  subnets        = [each.value[2]]
  mtu            = var.edge_node_uplink_mtu
...
#Rack 2
resource "nsxt_policy_bgp_neighbor" "fd2_bgp_neighbor_tor_a" {
  display_name          = "rack1_neighbor_tor_a"
  description           = "rack1_neighbor_tor_a"
  bgp_path              = nsxt_policy_tier0_gateway.infra_t0.bgp_config[0].path
  allow_as_in           = true
  ...
  hold_down_time        = var.bgp_timers["timer"]
  keep_alive_time       = var.bgp_timers["keepalive"]
  neighbor_address      = var.edge_node_bgp_config["fd2_bgp_peers"]["tor_a"]
  password              = "Password01!"
  remote_as_num         = var.bgp_asn["remote"]
...
resource "nsxt_policy_bgp_neighbor" "fd2_bgp_neighbor_tor_b" {
  display_name          = "rack2_neighbor_tor_b"
  description           = "rack2_neighbor_tor_b"
  bgp_path              = nsxt_policy_tier0_gateway.infra_t0.bgp_config[0].path
  allow_as_in           = true
  graceful_restart_mode = "DISABLE"
  hold_down_time        = var.bgp_timers["timer"]
  keep_alive_time       = var.bgp_timers["keepalive"]
  neighbor_address      = var.edge_node_bgp_config["fd2_bgp_peers"]["tor_b"]
  password              = "Password01!"
  remote_as_num         = var.bgp_asn["remote"]
...
data "nsxt_policy_tier1_gateway" "wld_t1_gateways" {
  for_each     = tomap(var.workload_domain_t1_networks)
  display_name = each.key
