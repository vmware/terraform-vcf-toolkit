...
variable "tier1_gateways" {
  description = "Tier-1 Logical-Routers. Key = Name, Value = Type (ECMP or HA)"
  type        = map(string)
  # {
  #   tenant1 = "ECMP"
  # }
...
variable "segments" {
  description = "NSX Segments to add.  "
  type = map(object({
    description  = optional(string, null)
    subnet_cidr  = optional(string, null)
    gateway_cidr = optional(string, null)
    t1           = optional(string, null) # if no Tier-1 is specified it will be L2 only, otherwise the Segment will be attached to the named Tier-1.
    advertise    = optional(bool, true)   # if this value is true, it will be added the outbound_prefix list.
  }))
}
